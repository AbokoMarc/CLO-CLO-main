...
async function initProduits() {
  ...
  function render() {
    ...
    grid.querySelectorAll(".btn-toggle-dispo").forEach(b => b.addEventListener("click", () => handleToggleDispo(parseInt(b.dataset.id), b.dataset.dispo === "true")));
  }

  async function handleToggleDispo(id, currentlyDispo) {
    try {
  ...
  async function handleDelete(id) {
    ...
    if (!confirm(`Supprimer définitivement "${product?.name}" ? Préférez "Marquer indisponible" si vous voulez juste le masquer temporairement — l'historique des commandes passées restera intact.`)) return;
    try {
...
async function initZones() {
  ...
  async function render() {
    ...
    list.innerHTML = zones.length
      ? zones.map(z => `
        <span style="background:white;border:1.5px solid #e5e7eb;border-radius:999px;padding:8px 14px;font-weight:700;font-size:0.85rem;display:inline-flex;align-items:center;gap:8px;">
          ${z.quartier} — ${z.ville}
          <button class="btn-delete-zone" data-id="${z.id}" style="background:none;border:none;color:#ef4444;font-weight:900;cursor:pointer;">×</button>
        </span>`).join("")
      : `<p style="color:#9ca3af;font-weight:700;">Aucune zone enregistrée.</p>`;

    list.querySelectorAll(".btn-delete-zone").forEach(btn => {
      btn.addEventListener("click", async () => {
        if (!confirm("Retirer cette zone de livraison ?")) return;
        try { await ProductService.removeZone(btn.dataset.id); render(); }
