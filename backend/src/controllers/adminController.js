...
export const AdminController = {
  ...
  /* ── ZONES DE LIVRAISON ── */
  async createZone({ req, res, body }) {
    requireRole(requireAuth(req), "admin");
    sendJson(res, 201, await AdminService.createZone(body));
  },
  async deleteZone({ req, res, params }) {
    requireRole(requireAuth(req), "admin");
    sendJson(res, 200, { deleted: await AdminService.deleteZone(params.id) });
  },

  /* ── SERVICE TRAITEUR ── */
  async listTraiteurRequests({ req, res }) {
    requireRole(requireAuth(req), "admin");
    sendJson(res, 200, await AdminService.listTraiteurRequests());
  },
  async updateTraiteurRequest({ req, res, params, body }) {
    requireRole(requireAuth(req), "admin");
    sendJson(res, 200, await AdminService.updateTraiteurRequest(params.id, body));
  },
