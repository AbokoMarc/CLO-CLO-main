...
export const AuthService = {
  ...
  /** Adresses de livraison enregistrées par le client (pour en choisir une rapidement
      à la commande, sans tout retaper) — libellé libre (ex: "Maison", "Bureau"). */
  async addFavoriteAddress(userId, { label, quartier, adresse }) {
    if (!quartier || !adresse) {
      const e = new Error("Quartier et adresse requis.");
      e.status = 400;
      throw e;
    }
    const user = await Store.findById("users", userId);
    const list = Array.isArray(user.favoriteAddresses) ? user.favoriteAddresses : [];
    list.push({ id: Date.now(), label: label || quartier, quartier, adresse });
    const updated = await Store.update("users", userId, { favoriteAddresses: list });
    return publicUser(updated);
  },
  async removeFavoriteAddress(userId, addressId) {
    const user = await Store.findById("users", userId);
    const list = (Array.isArray(user.favoriteAddresses) ? user.favoriteAddresses : [])
      .filter((a) => String(a.id) !== String(addressId));
    const updated = await Store.update("users", userId, { favoriteAddresses: list });
    return publicUser(updated);
  },
