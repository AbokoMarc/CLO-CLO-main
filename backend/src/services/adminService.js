...
export const AdminService = {
  ...
  /* ── ZONES DE LIVRAISON ── */
  async createZone({ ville, quartier }) {
    if (!ville || !quartier) {
      const e = new Error("Ville et quartier requis.");
      e.status = 400;
      throw e;
    }
    return Store.insert("zones", { ville: ville.trim(), quartier: quartier.trim() });
  },
  deleteZone(id) {
    return Store.remove("zones", id);
  },
